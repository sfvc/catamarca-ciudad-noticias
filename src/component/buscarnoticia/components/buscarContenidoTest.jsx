...
const BuscarContenidoTest = ({
  ...
  const filter = {};
  const limitPage = 2
  const sortField = "titulo"; // o cualquier campo por el que quieras ordenar
  const sortOrder = sortDirection ? sortField : `-${sortField}`;
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      if (data.meta) {
        setTotalPages(Math.ceil(data.meta.filter_count / limitPage))
      }
      return data.data;
    } catch (error) {
  ...
  const { data: noticias = [], error, isLoading } = useQuery(
    ...
    ["noticiasDestacadas", selectedTags, searchTerm, selectedCategory, currentPage, sortDirection, lastNews], // Clave dinámica basada en selectedTags
    fetchNews,
    {
